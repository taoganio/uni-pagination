 ...
 * MongoDB 行
 */
public class MongoDocumentPaginationRow implements MongoPaginationRow, Serializable, PaginationResultSetMetadataSetter {

    private final Document document;
    private PaginationResultSetMetadata metadata;

    public MongoDocumentPaginationRow(Document document) {
    ...
    public <T> T getEmbedded(List<?> keys, Class<T> clazz) {
        return document.getEmbedded(keys, clazz);
    }

    @Override
    public <T> T getEmbedded(List<?> keys, T defaultValue) {
        return document.getEmbedded(keys, defaultValue);
    }
    ...
    public <T> List<T> getList(Object key, Class<T> clazz, List<T> defaultValue) {
        return document.getList(key, clazz, defaultValue);
    }
